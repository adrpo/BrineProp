...
Option Base 1
'
'Public Type GasDataRecord 'Coefficient data record for properties of ideal gases based on NASA source
...
'  R_s As Double 'specific gas constant
'  nM As Integer 'number of ions per molecule
'End Type

Public CO2 As New GasDataRecord
Public N2 As New GasDataRecord
Public CH4 As New GasDataRecord
Public H2 As New GasDataRecord
Public H2O As New GasDataRecord

'Sub DefineGasPhase()
'    Gases = Array(CO2, N2, CH4, H2O)
'    Gases(1) = CO2
'End Sub

' clumsy mapping of constants to make them available in the spreadsheet
...
' ideal gas coefficients from Modelica.Media.IdealGases.SingleGases.*
Sub DefineGasData() ' called by sub Start() in ThisWorkbook
    Debug.Print "DefineGasData"
    With CO2
        .name = "CO2"
        .MM = 0.0440095 '[kg/mol]
        .Hf = -8941478.54440518
    ...
    With N2
        ...
        .R_s = R / .MM '296.803386950531
        .nM = 1
    End With
    
    With CH4
        .name = "CH4"
        .MM = 0.01604246 '[kg/mol]
        .Hf = -4650159.63885838
        .h0 = 624355.740952447
        .Tlimit = 1000
        ...
        .R_s = R / .MM '518.279116793809
        .nM = 1
    End With
   
    With H2
        .name = "H2"
        .MM = 0.00201588 '1.00784 * 2 * 0.001 (from Modelica.Media.IdealGases.Common.SingleGasesData.H2)
        .Hf = 0
        .h0 = 4200697.46215052
        .Tlimit = 1000
        .alow = Array(40783.2321, -800.918604, 8.21470201, -0.01269714457, 0.00001753605076, -1.20286027E-08, 3.36809349E-12)
        .blow = Array(2682.484665, -30.43788844)
        .ahigh = Array(560812.801, -837.150474, 2.975364532, 0.001252249124, -0.000000374071619, 5.9366252E-11, -3.6069941E-15)
        .bhigh = Array(5339.82441, -2.202774769)
        .R_s = R / .MM '4124.487568704486);
        .nM = 2
    End With

    'H2O is defined in Brine
    GasDataSet = True
End Sub

Function solubility_CO2_pTX_Duan2006(p As Double, T As Double, Xin, p_gas)
    Dim solu ' mol/kg_H2O
    Dim X_H2O As Double
    solu = solubility_CO2_pTX_Duan2006_molality(p, T, Xin, p_gas, X_H2O)
    If VarType(solu) = vbString Then
      solubility_CO2_pTX_Duan2006 = solu
      Exit Function
    End If
    solubility_CO2_pTX_Duan2006 = solu * CO2.MM * X_H2O 'molality->mass fraction
End Function
Function solubility_CO2_pTX_Duan2006_molality(p As Double, T As Double, Xin, p_gas, Optional ByRef X_H2O As Double) 'CO2 solubility in aqueous saltsolutions
...
'' fugacity from doi10.1016/j.marchem.2005.09.001
    Dim T_min As Double: T_min = 273.15
    Dim T_max As Double: T_max = 300 + 273.15
    Dim p_min As Double: p_min = 0
    Dim p_max As Double: p_max = 2000 * 10 ^ 5
    Dim b_max As Double: b_max = 4.5

   
    If Not p_gas > 0 Then
       solubility_CO2_pTX_Duan2006_molality = 0
       Exit Function
    ElseIf p_gas < 0 Then
       solubility_CO2_pTX_Duan2006_molality = "#p_gas negative! (GasProps.solubility_CO2_pTX_Duan2006_molality)"
       Exit Function
    ElseIf p_gas > p Then
       solubility_CO2_pTX_Duan2006_molality = "#p_gas > p ! (GasProps.solubility_CO2_pTX_Duan2006_molality)"
       Exit Function
    End If
    
    If p < 0 Then
       solubility_CO2_pTX_Duan2006_molality = "#p negative! (GasProps.solubility_CO2_pTX_Duan2006_molality)"
       Exit Function
    End If
    ' p_max & T_max is checked in fugacity subfunction
    ...
    If VarType(p_H2O) = vbString Then 'if error
        solubility_CO2_pTX_Duan2006_molality = p_H2O & "(solubility_CO2_pTX_Duan2006_molality)"
        Exit Function
    End If
    Dim phi
    ...
    If VarType(x) = vbString Then
       solubility_CO2_pTX_Duan2006_molality = x
       Exit Function
    End If
    X_H2O = x(Brine.nX) ' to be used in solubility_CO2_pTX_Duan2006 avoiding double calculation
 ...
 'constant
    ...
    Dim msg As String
    Dim b As Double: b = Application.Max(Array(m_Na, m_K, m_Ca, m_Mg)) ' ionic strength
    msg = RangeCheck_pTb(p, T, b, ignoreLimitCO2_p, ignoreLimitCO2_T, ignoreLimitCO2_b, p_min, p_max, T_min, T_max, b_max, "GasProps.solubility_CO2_pTX_Duan2006_molality")
    If Len(msg) > 0 Then
        solubility_CO2_pTX_Duan2006_molality = msg
        Exit Function
    End If
    ...
    If VarType(phi) = vbString Then 'if error
        solubility_CO2_pTX_Duan2006_molality = phi & "(solubility_CO2_pTX_Duan2006_molality)"
        Exit Function
    End If
    ...
    solubility_CO2_pTX_Duan2006_molality = phi * p_gas / 10 ^ 5 * Exp(-mu_l0_CO2_RT - 2 * lambda_CO2_Na * (m_Na + m_K + 2 * m_Ca + 2 * m_Mg) - zeta_CO2_NaCl * m_Cl * (m_Na + m_K + m_Mg + m_Ca) + 0.07 * m_SO4 * 0)
End Function
...
'        msg = "T=" & T - 273.15 & "°C, CO2 fugacity only valid for " & T_C_min & "<T<" & T_C_max & "°C (GasProps.fugacity_CO2_Duan2006)"
'    End If
'    If (p < 0 Or p > p_bar_max * 10 ^ 5) Then
'         msg = "p=" & p / 10 ^ 5 & " bar, CO2 solubility only valid for 0<p<" & p_bar_max & " bar (GasProps.fugacity_CO2_Duan2006)"
'    End If
'    If Len(msg) > 0 Then
...
End Function
 ...
 Function solubility_N2_pTX_Mao2006_molality(p As Double, T As Double, Xin, p_gas, Optional ByRef X_H2O As Double)   'solubility calculation of N2 in seawater Mao&Duan(2006)
    ...
    If Not p_gas > 0 Then
       ...
       Exit Function
    ElseIf p_gas < 0 Then
       solubility_N2_pTX_Mao2006_molality = "#p_gas negative! (GasProps.solubility_N2_pTX_Mao2006_molality)"
       Exit Function
    ElseIf p_gas > p Then
       solubility_N2_pTX_Mao2006_molality = "#p_gas > p ! (GasProps.solubility_N2_pTX_Mao2006_molality)"
       Exit Function
    End If
    
    If p < 0 Then
       solubility_N2_pTX_Mao2006_molality = "#p negative! (GasProps.solubility_N2_pTX_Mao2006_molality)"
       Exit Function
    End If
    ' p_max & T_max is checked in fugacity subfunction TODO
    
    Dim p_min As Double: p_min = 1 * 10 ^ 5
    Dim p_max As Double: p_max = 600 * 10 ^ 5
    Dim T_min As Double: T_min = 273.15
    Dim T_max As Double: T_max = 400
    Dim b_max As Double: b_max = 6
    ...
    If VarType(x) = vbString Then
       ...
       Exit Function
    End If
    X_H2O = x(Brine.nX) ' to be used in solubility_N2_pTX_Mao2006_molality
...
'          msg = "#T=" & (T - 273.15) & " °C, N2 solubility only valid for 0<T<127°C (GasProps.solubility_N2_pTX_Mao2006_molality)"
'       End If
'       If (p < 10 ^ 5 Or p > 600 * 10 ^ 5) Then
'          msg = "#p=" & (p / 10 ^ 5) & " bar, N2 solubility only valid for 1<p<600 bar (GasProps.solubility_N2_pTX_Mao2006_molality)"
'       End If
'       If molalities(i_NaCl) > 6 Then
'         msg = "#mola(i_NaCl)=" & (molalities(i_NaCl)) & " mol/kg, but N2 solubility only valid up to 6 mol/kg (GasProps.solubility_N2_pTX_Mao2006_molality)"
'       End If
'         If Len(msg) > 0 Then
'            If outOfRangeMode = 1 Then
'                Debug.Print msg
...
'     End If
    Dim b As Double: b = Application.Max(Array(m_Na, m_K, m_Ca, m_Mg)) ' ionic strength
    Dim msg As String
    msg = RangeCheck_pTb(p, T, b, ignoreLimitN2_p, ignoreLimitN2_T, ignoreLimitN2_b, p_min, p_max, T_min, T_max, b_max, "GasProps.solubility_N2_pTX_Mao2006_molality")
    If Len(msg) > 0 Then
        solubility_N2_pTX_Mao2006_molality = msg
        Exit Function
    End If
    ...
    If VarType(p_H2O) = vbString Then 'if error
        solubility_N2_pTX_Mao2006_molality = p_H2O & "(GasProps.solubility_N2_pTX_Mao2006_molality)"
        Exit Function
    End If
    X_NaCl = molalities(i_NaCl) * H2O.MM 'mole fraction of NaCl in liquid phase
    Dim rho_l_H2O: rho_l_H2O = IAPWS.Density_pT(p, T)
    If VarType(rho_l_H2O) = vbString Then 'if error
        solubility_N2_pTX_Mao2006_molality = rho_l_H2O & "(GasProps.solubility_N2_pTX_Mao2006_molality)"
        Exit Function
    End If
    Dim v_l_H2O As Double 'MolarVolume
    v_l_H2O = H2O.MM / rho_l_H2O
...
Function solubility_N2_pTX_Mao2006(p As Double, T As Double, Xin, p_gas) 'solubility calculation of N2 in seawater Mao&Duan(2006)
    ...
    If VarType(solu) = vbString Then
      ...
      Exit Function
    End If
    solubility_N2_pTX_Mao2006 = solu * N2.MM * X_H2O 'molality->mass fraction
End Function
...
' Todo Range
    Dim p_bar As Double, a
    p_bar = p / 10 ^ 5
    a = Array(0.00186357885, 0.0117332094, 0.000000782682497, -0.0000115662779, -3.13619739, -0.00129464029)
    fugacity_H2O_Mao2006N2 = Exp(a(1) + a(2) * p_bar + a(3) * p_bar ^ 2 + a(4) * p_bar * T + a(5) * p_bar / T + a(6) * p_bar ^ 2 / T) 'equ. 5
 End Function

Function solubility_CH4_pTX_Duan2006(p As Double, T As Double, Xin, p_gas)
    Dim solu ' mol/kg_H2O
    Dim X_H2O As Double
    solu = solubility_CH4_pTX_Duan2006_molality(p, T, Xin, p_gas, X_H2O)
    If VarType(solu) = vbString Then
      solubility_CH4_pTX_Duan2006 = solu
      Exit Function
    End If
    solubility_CH4_pTX_Duan2006 = solu * CH4.MM * X_H2O 'molality->mass fraction
End Function
Function solubility_CH4_pTX_Duan2006_molality(p As Double, T As Double, Xin, p_gas, Optional ByRef X_H2O As Double) 'Duan ZH, Mao SD. (2006) A thermodynamic model for calculating methane solubility, density and gas phase composition of methane-bearing aqueous fluids from 273 to 523 K and from 1 to 2000 bar. Geochimica et Cosmochimica Acta, 70 (13): 3369-3386.
...
'  output SI.MassFraction c_gas "gas concentration in kg_gas/kg_H2O"
    ...
    Dim p_max As Double: p_max = 2000 * 10 ^ 5
    Dim T_min As Double: T_min = 273.15
    Dim T_max As Double: T_max = 250 + 273
    Dim b_max As Double: b_max = 6

    
    If Not p_gas > 0 Then
       solubility_CH4_pTX_Duan2006_molality = 0
       Exit Function
    ElseIf p_gas < 0 Then
       solubility_CH4_pTX_Duan2006_molality = "#p_gas negative! (GasProps.solubility_CH4_pTX_Duan2006_molality)"
       Exit Function
    ElseIf p_gas > p Then
       solubility_CH4_pTX_Duan2006_molality = "#p_gas > p ! (GasProps.solubility_CH4_pTX_Duan2006_molality)"
       Exit Function
    End If
    
    If p < 0 Then
       solubility_CH4_pTX_Duan2006_molality = "#p negative! (GasProps.solubility_CH4_pTX_Duan2006_molality)"
       Exit Function
    End If
    ' p_max & T_max is checked in fugacity subfunction
    ...
    If VarType(rho_H2O) = vbString Then 'if error
        solubility_CH4_pTX_Duan2006_molality = rho_H2O & "(GasProps.solubility_CH4_pTX_Duan2006_molality)"
        Exit Function
    End If
    Dim v_l_H2O As Double: v_l_H2O = H2O.MM / rho_H2O 'MolarVolume
    ...
    If VarType(phi_H2O) = vbString Then 'if error
        solubility_CH4_pTX_Duan2006_molality = phi_H2O & "(GasProps.solubility_CH4_pTX_Duan2006_molality)"
        Exit Function
    End If
    Dim phi_CH4, mu_l0_CH4_RT As Double, lambda_CH4_Na As Double, xi_CH4_NaCl As Double
    
    Dim x
    x = CheckMassVector(Xin, Brine.nX)
    If VarType(x) = vbString Then
        solubility_CH4_pTX_Duan2006_molality = x
        Exit Function
    End If
    X_H2O = x(Brine.nX) ' to be used in solubility_CO2_pTX_Duan2006 avoiding double calculation
    ...
    Dim msg As String
    Dim b As Double: b = Application.Max(Array(m_Na, m_K, m_Ca, m_Mg)) ' ionic strength
    msg = RangeCheck_pTb(p, T, b, ignoreLimitCH4_p, ignoreLimitCH4_T, ignoreLimitCH4_b, p_min, p_max, T_min, T_max, b_max, "GasProps.solubility_CH4_pTX_Duan2006_molality")
    If Len(msg) > 0 Then
        solubility_CH4_pTX_Duan2006_molality = msg
        Exit Function
    End If
    
    phi_CH4 = fugacity_CH4_Duan1992(p_gas + p_H2O, T)
    If VarType(phi_CH4) = vbString Then
        solubility_CH4_pTX_Duan2006_molality = phi_CH4
        Exit Function
    End If
    ...
    'equ. 10
    Dim solu As Double
    solubility_CH4_pTX_Duan2006_molality = p_gas / 10 ^ 5 * phi_CH4 * Exp(-mu_l0_CH4_RT - 2 * lambda_CH4_Na * (m_Na + m_K + 2 * m_Ca + 2 * m_Mg) - xi_CH4_NaCl * (m_Na + m_K + 2 * m_Ca + 2 * m_Mg) * (m_Cl + 2 * m_SO4) - 4 * 0.0332 * m_SO4)
End Function

  
Function fugacity_CH4_Duan1992(p As Double, T As Double)    'Zero search with EOS from Duan1992
'    Dim msg As String
'    If Not VLEisActive And outOfRangeMode > 0 Then
'        If 273 > T Or T > 273 + 250 Then
'            msg = "#T=" & (T) & " K, CH4 solubility only valid for 0<T<250°C (GasProps.fugacity_CH4_Duan1992)"
'        End If
'        If p < 10 ^ 5 Or p > 2000 * 10 ^ 5 Then
'            msg = "#p=" & (p / 10 ^ 5) & " bar, but CH4 fugacity only valid for 1<p<1600 bar (GasProps.fugacity_CH4_Duan1992)"
'        End If
'
'        If Len(msg) > 0 Then
'            If outOfRangeMode = 1 Then
...
'    Dim msg As String
'    Dim b As Double: b = Application.Max(Array(m_Na, m_K, m_Ca, m_Mg)) ' ionic strength
'    msg = RangeCheck_pTb(p, T, b, ignoreLimitH2_p, ignoreLimitCO2_T, ignoreLimitCO2_b, p_min, p_max, T_min, T_max, b_max, "GasProps.solubility_CO2_pTX_Duan2006")
'    If Len(msg) > 0 Then
...
'    End If
    ...
    'equ. 6
    fugacity_H2O_Duan2006CH4 = Exp(a(1) + a(2) * p_bar + a(3) * p_bar ^ 2 + a(4) * p_bar * T + a(5) * p_bar / T + a(6) * p_bar ^ 2 / T)
End Function
...
Function MoistAirDynamicViscosity(T)
    ...
    If T < 240 Or T > 400 Then
        msg = "T out of range (240..400 K) for MoistAir viscosity calculation (GasProps.MoistAirDynamicViscosity)"
        If outOfRangeMode = 1 Then
            Debug.Print msg
        ElseIf outOfRangeMode = 2 Then
